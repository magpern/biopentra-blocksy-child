<?php
/**
 * Edit account form — read-only email for My Account v2.
 *
 * WooCommerce core 11.0.0, unchanged except the email field: My Account
 * v2's own copy (my-address.php's "Email address" card) tells customers
 * order history is matched to this address and changing it means
 * contacting support, so leaving this field editable here would
 * contradict that and let a change slip through a different form.
 * Blocksy_Child_My_Account_V2::guard_account_email() enforces this
 * server-side too — this template is the UI half, not the only guard.
 *
...
<form class="woocommerce-EditAccountForm edit-account" action="" method="post" <?php do_action( 'woocommerce_edit_account_form_tag' ); ?> >
	...
	<p class="woocommerce-form-row woocommerce-form-row--wide form-row form-row-wide">
		...
		<?php if ( $use_v2_layout ) : ?>
			<input type="email" class="woocommerce-Input woocommerce-Input--email input-text" name="account_email" id="account_email" autocomplete="email" value="<?php echo esc_attr( $user->user_email ); ?>" aria-required="true" readonly aria-describedby="account_email_description" />
			<span id="account_email_description" class="bp-ma-v2__hint">
				<?php
				printf(
					/* translators: %s: contact page URL */
					wp_kses(
						__( 'Your order history is matched to this address, so it can\'t be changed here. To change it, <a href="%s">contact support</a> and your past orders will move with you.', 'blocksy-child' ),
